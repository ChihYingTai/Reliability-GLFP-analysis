function [par_mle, output_result] = MLE_opt(data, initial_par)
% MLE: parameter estimation using MLE with incomplete likelihood function.
% usage: [par_mle, output_result] = MLE_opt(data, initial_par)
% 
% arguments: (input)
...
%  [par_mle, output_result] = MLE_opt(data, initial_par)
%
%
    iter_mle = 10;
...
%     options = optimset('MaxIter', 5000000, 'MaxFunEvals', 100000,'TolX',1e-10,'TolFun',1e-10);
    while i<=iter_mle
        try
            ...
            [par_opt, fval, ~, ~, ~, ~, hessian] = fmincon(@(x) MLE_opt_func(x, data), x0, A, b, Aeq, beq, lb, ub, []); % , options
...
function [par_mle, output_result] = opt_summary(par)
    % Parameters
    p = par(1);
    a1 = par(2); b1 = par(3);
    a2 = par(4); b2 = par(5);
